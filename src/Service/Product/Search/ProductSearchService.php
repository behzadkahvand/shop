...
final class ProductSearchService
{
    ...
    public function search(SearchData $data, Pagination $pagination): SearchResult
    {
        ...
        $dataEvent = new ProductSearchDataEvent($driverFQN, $data, $pagination);
        $this->dispatcher->dispatch($dataEvent);

        $query = $this->searchDriver->getQuery($dataEvent->getData(), $dataEvent->getPagination());

        $queryEvent = new ProductSearchQueryEvent($driverFQN, $query, $data, $pagination);
        ...
        $searchResult = new SearchResult($result, $searchQuery->getMeta());
        $this->dispatcher->dispatch(new ProductSearchResultEvent($driverFQN, $searchResult, $data));
